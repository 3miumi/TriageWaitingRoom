...
// Filename: p3.cpp
// Date :05/30/2022
// Purpose: This program is a priority queue system (triage) for a hospital emergency room.
// The levels of priority for the patients are the following: "immediate", "emergency", "urgent", "minimal"
// The triage user will determine the patient's priority based on their injury or illness and enter the patient's full
// name along with the urgency level to this program. The interface of the program is a command-line prompt that
// supports the following commands:
// Input:   User input of patient name and priority code
// Process: Patient objects for patients and priorityHeap class will be created in this program to store information.
// Global functions created to achieve menu option.
// Output:  Display corresponding information according to user's choice.
...
void addPatientCmd(string line, PatientPriorityQueue &priQueue) {
    ...
    const vector<string> V = {"immediate", "emergency", "urgent", "minimal"};
    ...
    if (strBegin == string::npos) {
        ...
        const auto strEnd = name.find_last_not_of(" \t");
...
//  validate priority is between 1 and 4
    if (find(V.begin(), V.end(), priority) == V.end()) {
        cout << "Error: priority code invalidate.\n";
        return;
    }
// add patient
...
void removePatientCmd(PatientPriorityQueue &priQueue) {
    ...
    if (priQueue.size() > 0) {
        cout << "This patient will now be seen: " << priQueue.peek().getName() << endl;
        priQueue.remove();
    } else
        cout << "There are no patients in the waiting area." << endl;
...
void welcome() {
    cout << "Welcome to Triage System!" << endl;
}
